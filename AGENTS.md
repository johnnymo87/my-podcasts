...
- Shared opencode-serve report mechanics: `pipeline/report_engine.py` (session lifecycle, `<script>`/`<summary>` extraction, publish-boundary refusals) — the **only** implementation, used by `transcript_report.py`, `report_writer.py`, `rundown_writer.py`, and `fp_writer.py`. Split into `fetch_report_text` (session lifecycle) and `parse_report` (extraction + every refusal), with `run_report_prompt` as their composition; the daily writers compose the halves themselves so they can persist raw model output between the two
...
**Refusals at the publish boundary.** `report_engine.parse_report` rejects an empty script, a script below `min_chars` (**2000** for transcript reports, 500 for the daily writers, unset for the operator-reviewed one-off path), any script with leaked `<script>`/`<summary>`/`<covered>` markup, and — when the caller passes `require_tags=True` — a reply carrying no `<script>` tag at all. Emptiness alone is the wrong test: on 2026-08-18 an FP Digest episode shipped as a 2636-byte mp3 because a 3-character `...` placeholder is not empty. `report_engine.extract_script` also picks the **longest** `<script>` block and recovers an unclosed final one, so a planning placeholder or a mangled closing tag cannot become the episode.

`require_tags` exists because the no-tag fallback is **not** a safety net. It returns essentially the raw reply with the `<summary>` block and stray tags stripped — measured at 6522 chars in, 6521 out. Cosmetic. So the model's own *reasoning* gets narrated as the episode, and no `min_chars` floor can catch it because raw output is long. Every automated publish path (both daily writers, the transcript path) passes `require_tags=True`. `report_writer` deliberately does not: those episodes are operator-reviewed before shipping, so a human is the guard.

**`require_tags` asks whether a script was *delimited*, not whether the tag is mentioned.** The naive substring version of this check shipped and was broken by adversarial review within the hour: a reply reading `…I need to produce a <script> tag now. Let me think. <3300 chars of reasoning>` satisfies "contains `<script>`", then falls into `extract_script`'s unclosed-tail branch and returns the reasoning itself — clean of markup, far above any floor, published. `_has_trustworthy_script_markup` therefore requires positive evidence: a well-formed `<script>…</script>` pair, **or** a trailing unclosed tag ending in a *mangled* close (`</scrip>`, the `ne0` shape). It is a disjunction on purpose — demanding a well-formed `</script>` would re-break `ne0` recovery, which is the entire reason the unclosed-tail branch exists.

**Raw-output persistence is atomic** (`report_engine.persist_raw_output`: write `.tmp`, then `os.replace`). The daily writers persist the model's reply before parsing so a retry can skip a 900-second regeneration, which makes that file an *input* to a later run. A plain `write_text` truncated mid-`<script>` yields, on the next retry, a clean-looking **half script** that passes every refusal — 3072 chars of real episode text ending mid-sentence, shipped silently. An empty file self-heals loudly; a half-written one does not, which is backwards. Deploying this pipeline is a `systemctl restart` of the very process doing the write, so the trigger is routine rather than exotic.
