...
def clean_html(html_content: str) -> str:
    ...
    # 2. Insert paragraph separators by adding extra newlines before <p>
    for p_tag in soup.find_all("p"):
        p_tag.insert_before("\n\n")  # double newlines

    # 3. Get text. Using no separator or a single space is fine,
    #    but we generally want to preserve the newlines we inserted.
    text_content = soup.get_text()

    # 4. Remove artificial line breaks from quoted-printable
    #    (i.e., the '=  \n' style breaks).
    text_content = re.sub(r"=\s*\n", "", text_content)

    # 5. Collapse runs of blank lines into a single double-newline
    text_content = re.sub(r"\n\s*\n\s*\n+", "\n\n", text_content)

    # 6. Convert runs of spaces/tabs (but not newlines) into a single space.
    #    This pattern means “one or more whitespace, excluding newlines.”
    text_content = re.sub(r"[^\S\r\n]+", " ", text_content)

    # 7. Trim trailing spaces before newlines, so we don’t end up with " \n"
    text_content = re.sub(r" +(\n)", r"\1", text_content)

    # 8. Finally, strip leading/trailing spaces and newlines
