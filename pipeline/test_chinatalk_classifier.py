...
def test_is_transcript_returns_true_on_yes(monkeypatch):
    ...
    with patch("pipeline.chinatalk_classifier.genai") as mock_genai:
        ...
        assert (
            is_transcript("Speaker A: Hello\nSpeaker B: Hi", "ChinaTalk: Episode 42")
            is True
        )
        ...
        assert "ChinaTalk: Episode 42" in call_kwargs["contents"]
        assert "Speaker A: Hello" in call_kwargs["contents"]
...
@pytest.mark.parametrize("response_text", ["yes", " YES ", "Yes"])
def test_is_transcript_case_insensitive(monkeypatch, response_text):
    monkeypatch.setenv("GEMINI_API_KEY", "fake-key")
    mock_response = MagicMock()
    ...
    with patch("pipeline.chinatalk_classifier.genai") as mock_genai:
        ...
        assert is_transcript("body", "subject") is True
